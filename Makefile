...
#
# Generated on 2026-07-01T04:59:13Z by kres 7421bcd.
...
.PHONY: e2e-test
e2e-test: image-talos-exporter
	@$(MAKE) image-talos-exporter PUSH=true
	docker pull $(REGISTRY)/$(USERNAME)/talos-exporter:$(IMAGE_TAG)
	$(ARTIFACTS)/talosctl cluster create e2e --name=talos-exporter-e2e --kubernetes-version=v1.35.0 --mtu=1450 --talosconfig-destination=$(ARTIFACTS)/talosconfig
	$(ARTIFACTS)/talosctl kubeconfig $(ARTIFACTS)/kubeconfig --talosconfig=$(ARTIFACTS)/talosconfig --nodes=10.5.0.2 --force
	docker run --rm --net=host --cap-drop=all --cap-add=DAC_OVERRIDE --userns=host -v /var/run:/var/run -v $(PWD)/$(ARTIFACTS)/:/out/ $(REGISTRY)/$(USERNAME)/talos-exporter:$(IMAGE_TAG) /talos-exporter --talosconfig=/out/talosconfig --listen=:9104 &
	sleep 3
	...
	kill %1 || true
	$(ARTIFACTS)/talosctl cluster destroy --name=talos-exporter-e2e
